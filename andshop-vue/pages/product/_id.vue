          ...
          <div class="col-lg-8">
            <div class="product_details_right_one">
              <div class="modal_product_content_one">
                <h3 class="text-capitalize" v-if="getDetail">{{ getDetail.title }}</h3>
...
export async function asyncData({ store, params }) {
...
export default {
  ...
  computed: {
    ...
    swiper() {
      return this.$refs.mySwiper?.swiper; // ? işareti koymak da güvenli olur
    },
  },
  ...
  methods: {
    getImageUrl(path) {
  if (!path) return "";
  return path.startsWith("http") ? path : `https://localhost:7032/${path}`;
},
...
for (let i = 0; i < variants.length; i++) {
  if (!uniqColor.includes(variants[i].color)) {
